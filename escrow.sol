/**
 *Submitted for verification at testnet.bscscan.com on 2023-03-27
*/
...
contract NewEscrow is ERC165 {
    ...
    function currentHolder(uint256 _tokenId, uint256 _editionNumber, IERC1155 _nftCollection)
        public
        view
        returns (address)
    {
        ...
        if (holder[_tokenId][_editionNumber] == address(0)) {
            (address creator1, , , ) = _nftCollection.ownerOfToken(_tokenId);
            return creator1;
        }
        ...
        returns (bool)
    {
        ...
        if(_order.nftCollection==ERC1155Interface){
        require(
            _editionNumber > 0 &&
        ...
        require(
            currentHolder(_order.tokenId, _editionNumber,_order.nftCollection) == _order.seller ||
                currentHolder(_order.tokenId, _editionNumber,_order.nftCollection) == address(this),
            "Already sold"
        );
        ...
        require(
            buyNowPayment(_order, _editionNumber, _order.pricePerNFT,_order.paymentToken != address(0),msg.sender ),
            "Payment failed"
        );
        _order.nftCollection.safeTransferFrom(
            address(this),
            msg.sender,
            _order.tokenId,
            1,
    ...
    function buyNowPayment(
        ...
        address _creator;uint256 _percent1; address _coCreator;
        if (_order.nftCollection == ERC1155Interface){
        (
            _creator,
            _percent1,
        ...
        ) = _order.nftCollection.ownerOfToken(_tokenId);
        }else{
            _creator = _order.seller;
            _percent1 = 100;
            _coCreator = address(0);
        }
        Cuts memory _cuts;
        if (_order.nftCollection == ERC1155Interface && !secondHand[_tokenId][editionNumber]) {
       uint256 flexFee = flexPlatFee[_tokenId];
            if (flexPlatFee[_tokenId] > 0) {              
                _cuts.platformCut = payAmount.mul(flexFee).div(100);
            } else {
                _cuts.platformCut = payAmount.mul(10).div(100);
            }
            _cuts.creatorsCut = payAmount.sub(_cuts.platformCut);
            _cuts.creatorCut = _cuts.creatorsCut.mul(_percent1).div(100);
            _cuts.coCreatorsCut = _cuts.creatorsCut.sub(_cuts.creatorCut);
            secondHand[_order.tokenId][editionNumber] = true;
        } else {
            _cuts.platformCut = payAmount.mul(5).div(100);
            _cuts.creatorsCut = payAmount.sub(_cuts.platformCut);
            _cuts.creatorCut = _cuts.creatorsCut.mul(_percent1).div(100);
            _cuts.coCreatorsCut = _cuts.creatorsCut.sub(_cuts.creatorCut);
            
        }
        address _paymentToken = _order.paymentToken;
        if(isTokenPayment){
           tokenPay(_from, _creator, _cuts.creatorCut, _paymentToken);
        if (_cuts.coCreatorsCut > 0) {
            tokenPay(_from, _coCreator, _cuts.coCreatorsCut, _paymentToken);
        }
        tokenPay(_from, admin, _cuts.platformCut, _paymentToken);
        if (_cuts.finalCut > 0) {
            tokenPay(_from, _order.seller, _cuts.finalCut, _paymentToken);
        }       
        }else{
            sendValue(payable(_creator), _cuts.creatorCut);
            if (_cuts.coCreatorsCut > 0) {
                sendValue(payable(_coCreator), _cuts.coCreatorsCut);
            }
            _cuts.finalCut = payAmount.sub(
                _cuts.platformCut.add(_cuts.creatorCut).add(_cuts.coCreatorsCut)
            );
            if(_cuts.finalCut>0){sendValue(payable(_order.seller), _cuts.finalCut);}
            sendValue(admin, _cuts.platformCut);
        }
        return true;
    }
        ...
        returns (bool)
    {
        ...
        require(buyNowPayment(_order, _editionNumber, bidAmount, _order.paymentToken != address(0), address(this)));
        // }
    ...
    function putOnSaleBuy(
        ...
        uint256 _pricePerNFT,
        address _paymentToken,
        IERC1155 _nftCollection
    ) public returns (bool) {
        return placeSecondHandOrder(_tokenId, _editionNumber, _pricePerNFT, 2,_paymentToken, _nftCollection);
    }
    ...
    function placeSecondHandOrder(
        ...
        if(_nftCollection == ERC1155Interface){
         require(
            secondHand[_tokenId][_editionNumber],
            "Edition is not in second market"
        );
         require(
            currentHolder(_tokenId, _editionNumber, _nftCollection) == msg.sender,
        ...
        _nftCollection.safeTransferFrom(
            msg.sender,
            address(this),
            _tokenId,
            1,
    ...
    function requestOffer(
        ...
        return placeSecondHandOrder(_tokenId, _editionNumber, _pricePerNFT, 3,_paymentToken, _nftCollection);
    }
    ...
    function transfer(
        ...
        require(currentHolder(id, editionNumber, _nftCollection) == msg.sender, "Not owner");
        }
        
        
        _nftCollection.safeTransferFrom(from, to, id, 1, data);
        if(_nftCollection==ERC1155Interface){
    ...
    function burnTokenEdition(uint256 _tokenId, uint256 _editionNumber, IERC1155 _nftCollection)
        external
        returns (bool)
    {
        if( _nftCollection == ERC1155Interface ){
        ...
        _nftCollection.burn(msg.sender, _tokenId, 1);
